...
async function transform({ prisma, data }) {
    ...
    for (const f of FORMS) {
        ...
        const out = { sheet: f.sheet, label: f.label, model: f.model, modelMissing: !!(sc && sc.__missing), fkFields: f.fkFields || {}, requiredFields: required, rows: [], counts: { rows: 0, mapped: 0, errors: 0, warnings: 0 } };
        ...
        for (const r of rows) {
            ...
            if (rec.__error) {
                ...
                // mandatory-but-empty: the new site requires this field but the old data has nothing
                for (const rf of required) {
                    if (rf === 'reportingYearDate') continue; // derived from year, never user-facing
                    if (isEmpty(rec[rf]) && !(rec._fk && rec._fk[rf]) && !issues.some((i) => i.field === rf)) {
                        issues.push({ field: rf, message: 'Mandatory on the new site — empty in the old data, please fill', severity: 'warn' });
                    }
                }
            }

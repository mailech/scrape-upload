...
async function transform({ prisma, data }) {
    ...
    for (const f of FORMS) {
        ...
        const out = { sheet: f.sheet, label: f.label, model: f.model, modelMissing: !!(sc && sc.__missing), rows: [], counts: { rows: 0, mapped: 0, errors: 0, warnings: 0 } };
        ...
        rows.forEach((r) => {
            ...
            if (rec.__error) {
                ...
                // form-specific issues (master/dropdown misses, stashed totals)

    ...
    return [
        ...
        },
        {
            ...
            map: (r) => ({
                ...
                remark: S(r['VIP/Experts']),
            }),
            key: (d, kvkId) => ({ kvkId, name: d.name, dateOfVisit: d.dateOfVisit }),
            issues: (r) => S(r['VIP/Experts']) ? [ISS('remark', `Dignitary type "${S(r['VIP/Experts'])}" kept as a note (type list not matched)`)] : [],
        },
        {
            ...
            issues: (r) => S(r['PI/CO PI']) ? [ISS('piTypeId', `PI/Co-PI type "${S(r['PI/CO PI'])}" not stored — this model has no field for it`)] : [],
        },
        {
        ...
        },
        {
            ...
            issues: (r) => [
                ...seasonIssue(r, 'seasonId', 'left blank'),
                ...(S(r['Type']) ? [ISS('soilParameterId', `Type "${S(r['Type'])}" not linked (soil-parameter list not matched)`)] : []),
            ],
        },
        {
            ...
            issues: (r) => S(r['Name of Activity']) ? [ISS('activityId', `Activity "${S(r['Name of Activity'])}" not linked (activity list not matched)`)] : [],
        },
